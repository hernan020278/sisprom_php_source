/**
 * Autor: Rafael Alberto Moreno Parra Correo: ramsoftware@gmail.com Página Web:
 * http://darwin.50webs.com Licencia: LGPL Fecha: Enero de 2012
 */
function Pieza() {
	/*
	 * Nivel en la expresión, a mayor valor, es mayor profundidad en la
	 * expresión
	 */
	this.nivel;
	 ...
	 * @param nivel
	 *            Nivel en que se encuentra en la expresión este operador
	 */
	 ...
	 * 
	 * @param acumulado
	 *            Acumulado que nace de cada operación simple es dado a la pieza
	 *            aquí.
	 */
...
/**
 * Evaluador de expresiones algebraicas. Recibe la expresión algebraica por
 * ejemplo: "sin(x)+cos(y)*3.14" como una cadena (String) junto con el valor de
 * las variables (x=8.92, y=-7.43) el algoritmo interpreta esa expresión
 * respetando las reglas algebraicas y retorna el resultado de tipo double.
 * 
 * @author Rafael Alberto Moreno Parra Correo: ramsoftware@gmail.com Página Web:
 ...
 */
{
	...
	/**
	 * Lista simplemente enlazada que tiene los componentes (número, operador o
	 * función) de la expresión algebraica ya dividida.
	 */
	this.Piezas = [];
	/**
	 ...
	 * 
	 * Convierte una expresión algebraica en una sucesión de nodos. |2| |+| |a|
	 * |/| |5| |*| |cos| |y|
	 * 
	 * @param expr
	 *            La expresión algebraica sin espacios y en minúsculas
	 */
	this.Analizar = function(expr) {
		...
		for (cont = 0; cont < longitud; cont++) {
			...
			/* Si es una variable o una función */
				...
				if (cont < longitud - 1) {
					/*
					 * Chequea si el siguiente carácter es una letra, dado el
					 * caso es una función
					 */
			...
			/* Si es un número */
				...
				this.Piezas[NumeroPieza++] = objeto;
				/*
				 * Inicializa de nuevo las variables de conversión de string a
				 * número
				 */
				entero = fraccion = 0;
				divide = 1;
		...
		/*
		 * Cierra la expresión simple preguntando si el último operando es un
		 * número
		 */
		if (acumulabaNumero) {
			objeto = new Pieza();
	...
	/**
	 * Ya construida la lista del tipo: [nodo número] [nodo operador] [nodo
	 * número] [nodo operador] ..... es ir del operador con mas precedencia al
	 ...
	 * analizar de nuevo la expresión, luego es muy rápido evaluar múltiples
	 * veces la misma expresión.
	 * 
	...
	this.Evaluar = function() {
		...
		for (pos = 0; pos < totalPiezas; pos++) {
			 ...
			 * acumulado de cada pieza. ¿Cómo? Extrae el valor del arreglo
		...
		for (evaluaNivel = this.MaximoNivel; evaluaNivel >= 0; evaluaNivel--) {
			...
			for (pos = 0; pos < totalPiezas; pos++) {
				/*
				 * Si encuentra una pieza de tipo función la evalúa con el valor
			...
			for (pos = 0; pos < totalPiezas; pos++) {
				...
				if (this.Piezas[pos].getNivel() == evaluaNivel
					...
					/*
					 * Busca un nodo anterior que sea número o variable y no
					 * haya sido evaluado
					 */
					for (antes = pos - 1; this.Piezas[antes].isEvaluado(); antes--)
						;
					/*
					 * Busca un nodo siguiente que sea número o variable y no
					 * haya sido evaluado
					 */
			...
			for (pos = 0; pos < totalPiezas; pos++) {
				/*
				 * Si encuentra un nodo del tipo operador y es multiplicación o
				 * división
				 */
				if (this.Piezas[pos].getNivel() == evaluaNivel
						&& this.Piezas[pos].getTipo() == this.ESOPERADOR
	 ...
	 * 
	 * @param variable
	 *            ¿Qué variable de la expresión recibirá el valor? a..z
	 * @param valor
	 *            Valor de tipo double que tendrá esa variable.
	 */
