		...
		arti.import(function(){
			...
			Sisem.Cargar({
				...
				afterLoad:function(data){
					$main = $('#content');
					w.$e = $('#mainPanel');
					arti.iniciarBrowse($.extend(w,{idGrid:w.pag.idGridWB, showMenCtx: true, showToolBar: true, multiSelect: true, autoWidth: false}));
				}
			});
		});
	},
		...
		if(!Sisem.isEmpty(w.$e.find('[name=art_kyart]').val())){
			Sisem.ejecutar('adm/GetListaArticulo',{art_kyart: w.$e.find('[name=art_kyart]').val()}, function(rpta){
				if(rpta.lista.items.length > 0)
				{
					...
					Sisem.getFile('adm/'+fila.art_foto, 'imagen', function(rptaImg){
						if(rptaImg.msg.type=='success')
						{
							w.$e.find('[name=img_art_foto]').attr('src',rptaImg.listaArchivo[0]+'?'+(new Date()).getTime()).fadeIn();
						}
						else{w.$e.find('[name=img_art_foto]').attr('src','/gestion/sisprom/cmn/imagen/pregunta.png').fadeIn();}
//						w.$e.find('[name=img_fimg]').css('width',ancho+'px');
	...
	activarFormulario:function(w){
		...
		Sisem.activar(w.$e.find('[name=art_unid]'), false);
		...
		Sisem.activar(w.$e.find('[name=art_foto]'), false);
		Sisem.activar(w.$e.find('[name=btnSubirArt_foto]'), w.activar);		
		w.$e.find('[name=inp_art_foto]').css('display','none');
		...
		w.$e.find("[name="+prf_tabl+"_"+prf_camp+"]").on('keyup', function(event){
			...
			if(Sisem.isEmpty(valor) && ( charCode == 8 || charCode == 46 )){
				...
				else
				{
					...
					w.$e.find("[name="+prf_tabl+"_clas]").val('');
            ...
            if(idxEle==prf_tabl+"_"+prf_camp)
            {
              ...
              break;
				...
				else
				{
					...
					w.$e.find("[name="+prf_tabl+"_clas]").val(obj['art_clas']);
                    ...
                    if(idxEle==prf_tabl+"_"+prf_camp)
                    {
                      ...
                      break;
								...
								else
								{
									...
									w.$e.find("[name="+prf_tabl+"_marc]").val(objSel.art_marc);
									w.$e.find("[name="+prf_tabl+"_clas]").val(objSel.art_clas);
