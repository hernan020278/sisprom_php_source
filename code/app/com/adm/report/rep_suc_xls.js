				...
				$.get(base_url+'cmn/repo/printReport',data,function(rpta){
					if(rpta!=null){
						...
						if(rpta.data.lisSuc!=null){
							...
							//data rendering
							workSheetData.push(['',
							    {value:'ID_SUCU',metadata: {style: headerStyle.id}},
								{value:'NOMBRE',metadata: {style: headerStyle.id}}
							]);
							for(var i=0;i<rpta.data.lisSuc.length;i++){
								workSheetData.push(['',
								    {value:parseInt(rpta.data.lisSuc[i].id_sucu),metadata: {style: cellStyle.id}},
								    {value:rpta.data.lisSuc[i].nomb,metadata: {style: cellStyle.id}}
								]);
							}
