	...
	public function __construct() {parent::__construct();}
 	...
 	public function ejecutar($data)
 	{
 		...
 		try
 		{
			...
			if( $filAct == "cco_kycco"){$data['sql'] = "select ".$data['bal_nact']." bal_nact, cco.cco_kycco bal_kybal, concat(UPPER(SUBSTR(ent.usu_nomb,1,3)),cco_dscr,substr(cco_ndoc,-4),substr(cco_tcta,1,4),substr(cco.cco_tmon,1,3)) bal_prop,";}
        ...
        ifnull((select sum(opeTmp.ope_oimp) from adm_operacion opeTmp ".$whope." and ( opeTmp.ope_tope='COSTO' OR (opeTmp.ope_tope='TRANSFERENCIA' AND opeTmp.ope_otip='EGRESO') )".$feact."),0.00) bal_cost,
				ifnull((select sum(opeTmp.ope_oimp) from adm_operacion opeTmp ".$whope." and opeTmp.ope_tope='GASTO'".$feact."),0.00) bal_gast
