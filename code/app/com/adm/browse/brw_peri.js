var brw_peri = {
	ejecutar: function(w)
	{
		...
		var $grid = new Sisem.grid({
			...
			cols: colsTmp.concat([
			    {name: 'prp_secc', descr:'Tipo',width:60, search: 'true'},
		        {name: 'prp_prop', descr:'Propiedad',width:100, search: 'true'},
		        {name: 'prp_valu', descr:'Fec-Ini',width:70, search: 'true'},
		        {name: 'prp_dscr', descr:'Fec-Fin',width:70, search: 'true'},
		        {name: 'prp_esta', descr:'Estado',width:50, search: 'false'}
			]),
			onLoading: function(){
			...
			params: {archivo: w.archivo,  prp_kypdr: w.prp.prp_kyprp, prp_secc: w.prp.prp_secc, prp_nive: w.prp.prp_nive, orderColumn: 'prp_dscr', orderType: 'asc'},
...
//				_toolBarHtml+='<button name="btnLoad'+w.tipo.toLowerCase()+'Fxls" class="btn btn-primary"><i class="fa fa-upload"></i></button>',
			...
			onContentLoaded: function($el){
				$el.find('[name=btnAdd'+w.tipo.toLowerCase()+']').click(function(){
			    	var exito = false;
			    	var fila = {pdr_kyprp: 0, pdr_codi: 'PRPPDR', pdr_secc: 'PRPPDR', pdr_prop: 'PADRE', pdr_dscr: 'Descripcion padre'};
			    	if(w.tipo=='PRPPDR')
			    	{
			    		w.pdr = fila;
			    		prop.btnAddClick(w);
			    	...
			    	{
						Sisem.ejecutar('adm/GetListaPropiedad',{prp_secc: w.tipo, prp_nive: 1}, function(rpta){
							if(rpta.lista.items.length > 0)
							{
								fila = rpta.lista.items[0];
					    		w.pdr = fila;
					    		prop.btnAddClick(w);
							}
							else{Sisem.msgBox('error','La propiedad '+w.tipo+' no tiene PADRE!!!');}
						});
			    	}
				});
			...
			load: function(rpta,$tbody){
				if(rpta.items!=null){
					for (var key in rpta.items)
					{
						...
						$row.append('<td>'+fila.prp_secc+'</td>');
						$row.append('<td>'+fila.prp_prop+'</td>');
						$row.append('<td>'+fila.prp_valu+'</td>');
						$row.append('<td>'+fila.prp_dscr+'</td>');
						$row.append('<td>'+estado[fila.prp_esta].label+'</td>');
							    ...
							    menuSelected: function (invokedOn, selectedMenu) {
							    	...
							    	switch($id){
							    		...
							    		case w.idMenCtx+"_vhij":
							    			prop.winSel({
							    				tipo: $row.data('data').prp_secc,
							    				tipoBrowse: 'EXPLORADOR',
						    					prp: {
						    						prp_kyprp: $row.data('data').prp_kyprp,
						    						prp_kypdr: $row.data('data').prp_kyprp,
						    						prp_secc: $row.data('data').prp_secc
						    					},
												modo:'VISUALIZAR',
												size: 'short',
												callback:function(){
							    		...
							    		case w.idMenCtx+"_dele":
							    			...
							    			if(lisKySel.length==0){lisKySel.push({ky: $row.data('data').prp_kyprp});}
