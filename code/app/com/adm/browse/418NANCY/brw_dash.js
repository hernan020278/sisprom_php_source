	...
	ejecutar: function(w)
	{
		...
		var $grid = new Sisem.grid({
			...
			cols: colsTmp.concat([
				...
				{name: '', descr:'Costo',width:450, search: 'false'}
			]),
			onLoading: function(){
			...
			load: function(rpta,$tbody){
				if(rpta.items!=null){
					...
					for (var key in rpta.items)
					{
						...
						$row.append('<td align="right" >'+bal_cost+'</td>');
					...
					$row.append('<td align="right" style="color:'+genColor['AZUL']+'">'+Sisem.redondeoString(totaCost)+'</td>');
					...
					w.$e.find('[name=spntotaCost]').html(prp_prop+' '+Sisem.redondeoString(totaCost));
					...
					w.$e.find('[name=prgtotaCost]').css('width',parseInt( ( totaCost * 100 ) / impMayor )+'%');
