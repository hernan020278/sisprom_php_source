...
class pdfRepBal extends Controlador {
	...
	public function index(){
		...
		if($respNiv1!=false){
			foreach($respNiv1 as $fil1){
				...
				$y+=5;
				if($this->getY()>=275){$this->AddPage();$this->columnHeader($data);}
				...
				if($respNiv2!=false){
					foreach($respNiv2 as $fil2){
						...
						$y+=5;
						if($this->getY()>=275){$this->AddPage();$this->columnHeader($data);}
						$respNiv3=$this->documento->listaMix($id_sucu, $id_enti, $fil2->id_docu, '0002', $fini, $ffin);
						if($respNiv3!=false){
