...
class pdfRepEntCot extends Controlador {
	...
	public function index(){
		...
		$respNiv1=$this->documento->listaMix( $id_sucu, $id_enti, '', '0005', $fini, $ffin);
		if($respNiv1!=false){
			foreach($respNiv1 as $fil1){
				...
				$respNiv2=$this->documento->listaMix($id_sucu, $id_enti, $fil1->id_docu, '0006', $fini, $ffin);
				if($respNiv2!=false){
					foreach($respNiv2 as $fil2){
						...
						$y+=5;
						if($this->fpdf->getY()>=275){$this->fpdf->AddPage();$this->fpdf->columnHeader($data);}
						$respNivTmpUno=$this->documento->listaMix($id_sucu, $id_enti, $fil2->id_docu, '0007', $fini, $ffin);
						$respNivTmpDos=$this->documento->listaMix($id_sucu, $id_enti, $fil2->id_docu, '0008', $fini, $ffin);
						
						if(is_array($respNivTmpUno) && is_array($respNivTmpDos)){$respNiv3 =  array_merge($respNivTmpUno,$respNivTmpUno);}
						else if(is_array($respNivTmpUno) && !is_array($respNivTmpDos)){$respNiv3 =  $respNivTmpUno;}
						else if(!is_array($respNivTmpUno) && is_array($respNivTmpDos)){$respNiv3 =  $respNivTmpDos;}
						...
						if($respNiv3!=false){
							foreach($respNiv3 as $fil3){
								...
								$this->fpdf->setXY($x1+10,$y);$this->fpdf->MultiCell($w1-10,5,$tipOpe[$fil3->tope],'1','L');
								$this->fpdf->setXY($x2,$y);$this->fpdf->MultiCell($w2,5,$tipDoc[$fil3->tdoc],'1','R');
								$this->fpdf->setXY($x3,$y);$this->fpdf->MultiCell($w3,5,$fil3->ndoc,'1','R');
								$this->fpdf->setXY($x4,$y);$this->fpdf->MultiCell($w4,5,$fil3->enom,'1','R');
								$this->fpdf->setXY($x5,$y);$this->fpdf->MultiCell($w5,5,$fil3->freg,'1','R');
								$this->fpdf->setXY($x6,$y);$this->fpdf->MultiCell($w6,5,$fil3->dscr,'1','R');
								$this->fpdf->setXY($x7,$y);$this->fpdf->MultiCell($w7,5,number_format($fil3->cant,2),'1','R');
								$this->fpdf->setXY($x8,$y);$this->fpdf->MultiCell($w8,5,number_format($fil3->ccot,2),'1','R');
								$this->fpdf->setXY($x9,$y);$this->fpdf->MultiCell($w9,5,number_format($fil3->cped,2),'1','R');
								$this->fpdf->setXY($x10,$y);$this->fpdf->MultiCell($w10,5,number_format($fil3->cent,2),'1','R');
								$this->fpdf->setXY($x11,$y);$this->fpdf->MultiCell($w11,5,number_format($fil3->cfac,2),'1','R');
								$y+=5;
								if($this->fpdf->getY()>=275){$this->fpdf->AddPage();$this->fpdf->columnHeader($data);}
							}//foreach($respNiv3 as $fil3){
						}//if($respNiv3!=false){
