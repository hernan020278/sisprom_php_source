...
//						var fileNameNew='ope_'+(new Date()).getTime()+'.'+fileExtension;
					...
					archivos = [file];
					Sisem.uploadMultipleFiles(w, fileName, archivos, base_url+'erp/oper/uploadOperFxls', function(rpta){
...
//						var fileNameNew='ope_'+(new Date()).getTime()+'.'+fileExtension;
					...
					archivos = [file];
					Sisem.uploadMultipleFiles(w, fileName, archivos, base_url+'erp/oper/uploadDetrFxls', function(rpta){
						Sisem.msgBox(rpta.msg.type, rpta.msg.text);
						if(rpta.msg.type=='success' || rpta.msg.type=='warning')
						{
							if(w.idGrid){$('#'+w.idGrid).trigger('reloadGrid');}
...
//						var fileNameNew='ope_'+(new Date()).getTime()+'.'+fileExtension;
					...
					archivos = [file];
					Sisem.uploadMultipleFiles(w, fileName, archivos, base_url+'erp/oper/uploadConcFxls', function(rpta){
						Sisem.msgBox(rpta.msg.type, rpta.msg.text);
						if(rpta.msg.type=='success' || rpta.msg.type=='warning')
						{
							if(w.idGrid){$('#'+w.idGrid).trigger('reloadGrid');}
								    ...
								    menuSelected: function (invokedOn, selectedMenu) {
							    		...
							    		case w.idMenCtx+"_abri":
							    			$.SmartMessageBox({
												...
												if (ButtonPressed == "Si") {
													$.post(base_url+'erp/apci/deleteByTipo',{apc_kyapc:$row.data('data').apc_kyapc, otip: 'CIERRE'},function(rpta){
														Sisem.msgBox(rpta.msg.type, rpta.msg.text);
														if(w.idGrid){$('#'+w.idGrid).trigger('reloadGrid');}
//														apci.init();
													},'json');
...
//						    				var url = base_url+'erp/docu/print_documento?id_sucu='+$row.data('data').id_sucu+'&tdoc='+$row.data('data').tdoc+'&id='+$row.data('data').id_docu;
