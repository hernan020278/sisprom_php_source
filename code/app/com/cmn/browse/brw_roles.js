var brw_roles = {
	ejecutar: function(w)
	{
		...
		var $grid = new Sisem.grid({
			...
			params: {archivo: w.archivo, rol_kyusu: w.usu.usu_kyusu, rol_kycom: USERDATA.com.com_kycom, orderColumn: 'prf_nomb', orderType: 'asc'},
			itemdescr: roles.pag.alias+'(s)',
			// toolbarURL: '',
			...
			load: function(rpta,$tbody){
				if(rpta.items!=null)
				{
					for(var i=0;i<rpta.items.length;i++)
					{
						...
						$row.append('<td>'+$ite.prf_nomb+'</td>');
						$row.append('<td>'+$ite.prf_dscr+'</td>');
							    ...
							    menuSelected: function (invokedOn, selectedMenu) {
							    	...
							    	switch($id){
							    		case w.idMenCtx+"_dele":
							    			...
							    			var lisKyDel = [];
							    			for(var i=0; i<lisDel.length; i++){lisKyDel.push({rol_kyrol: lisDel[i].rol_kyrol, prf_nomb: lisDel[i].prf_nomb});}
							    			if(lisKyDel.length==0){lisKyDel.push({rol_kyrol: $row.data('data').rol_kyrol, prf_nomb: $row.data('data').prf_nomb});}
							    			var resp = Sisem.msgAsk('Desea eliminar', $row.data('data').prf_nomb, function(rpta){
								    			if(rpta=='Si'){
													var data = {comando: 'ELIMINAR', lisKy: lisKyDel};
													Sisem.ejecutar('cmn/CtrlRoles',data, function(rpta){
