var brw_tare = {
	ejecutar: function(w)
	{
		...
		var $grid = new Sisem.grid({
			...
			cols: colsTmp.concat([
			    {name:'tar_nomb', descr:'Nombre',width:154, search: 'true'},
			    {name:'tar_dscr', descr:'Descripcion',width:250, search: 'true'},
				{name:'tar_orde', descr:'Orde',width:80, search: 'true'},
				{name:'tar_nive', descr:'Nivel',width:80, search: 'true'},
   			]),
			onLoading: function(){
			...
			params: {archivo: w.archivo, tipo:w.tipo, tar_kypdr: w.tar.tar_kypdr, tar_nive: w.tar.tar_nive, orderColumn: 'tar_orde', orderType: 'asc'},
			itemdescr: tare.pag.alias+'(s)',
			// toolbarURL: '',
...
//					for(var i=0;i<rpta.items.length;i++){
					for (var key in rpta.items)
					{
						...
						$row.append('<td>'+fila.tar_nomb+'</td>');
						$row.append('<td>'+fila.tar_dscr+'</td>');
						$row.append('<td name="tdPolitica_'+fila.tar_kytar+'">'+fila.tar_orde+'</td>');
						$row.append('<td>'+fila.tar_nive+'</td>');
							    ...
							    menuSelected: function (invokedOn, selectedMenu) {
						    		...
						    		case w.idMenCtx+"_oper":
							    		...
							    		case w.idMenCtx+"_prin":
											var data = {comando: 'IMPRIMIR', tar_nomb: 'ERP_tbl_ELIMINAR', tbl_kytbl: $row.data('data').tbl_kytbl};						    			
					...
					$tbody.find('[name^=tdPolitica_]').click(function(e){
						...
						if($tdPoli.children().length == 0)
						{
							...
							$tdPoli.append('<input name="tar_orde_'+dataRow.tar_kytar+'" type="text" class="form-control input-xs">');
							$tdPoli.find('[name=tar_orde_'+dataRow.tar_kytar+']').on("focus", function (){$(this).select();});
							$tdPoli.find('[name=tar_orde_'+dataRow.tar_kytar+']').val(dataRow.tar_orde).focus();
							
							$tdPoli.find('[name=tar_orde_'+dataRow.tar_kytar+']').blur(function(e){
								Sisem.ejecutar('cmn/CtrlPolitica',{comando: 'MODIFICAR', tar_kytar: dataRow.tar_kytar, tar_orde: $tdPoli.find('[name=tar_orde_'+dataRow.tar_kytar+']').val()}, function(rpta){
									if(rpta.msg.type=='success')
									{
										$tdPoli.parent().data('data').tar_orde=$tdPoli.find('[name=tar_orde_'+dataRow.tar_kytar+']').val();
										dataRow = $tdPoli.parent().data('data');
										$(e.target).remove();
										$tdPoli.html(dataRow.tar_orde);
									}//if(rpta.msg.type=='success')
